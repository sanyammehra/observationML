  ...
  for root, dirs, files in os.walk(path, topdown=False):
    for name in files:
      if name.find('.png') != -1:
        ...
        match = re.search(r'(?P<frame_id>\d+).png', name)
...
def foresight(state, dist=16):
  ...
  im_paths = state.frame_paths[state.image_idx + 1: state.image_idx + 1 + dist]
  if len(im_paths) > 0:
...
def hindsight(state, dist=16):
  ...
  im_paths = state.frame_paths[max(state.image_idx - 1 - dist, 0): max(state.image_idx - 1, 0)]
  if len(im_paths) > 0:
    image_grid = grid_images(im_paths=im_paths, w=np.ceil(np.sqrt(dist)), h=np.ceil(np.sqrt(dist)), margin=1)
    cv2.imshow('Hindsight', image_grid)


def image_info(image, task_state, state):
  """
  ...
  Args:
    ...
    state (obj) : current VideoState object
  Returns:
  """
  image_info = 'Frame {}/{} ({})'.format(state.image_idx + 1, state.num_frames, state.get_image_name())
  cv2.putText(image, image_info, (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, state.color, 1)
  ...
  elif state.look_ahead == 1: # only foresight
    foresight(state)
  elif state.look_ahead == 2: # foresight and hindsight
    foresight(state)
    hindsight(state)
...
def print_info(task_state, state):
  """
  ...
  # state information
  blue_bg('\n              State               ')
  orange_fg('Task ID: ', '{}\t'.format(task_state.tasks[task_state.task_idx]), newline=False)
  orange_fg('Frame ID: ', '{}'.format(state.get_image_name()))
  orange_fg('Image ID: ', '{}/{}'.format(state.image_idx + 1, state.num_frames))
  orange_fg('Action ID: ', state.get_image_label())
  ...
  # annotations
  blue_bg('\n           Actions Record            ')
  for frame_idx, (f, a) in enumerate(state.labels.items()):
...
def read_key(key, task_state, state):
  """
  ...
  if key in KEYMAP['left']:   # -1 frame
    state.image_idx -= 1
    state.image_idx = min(max(0, state.image_idx), state.num_frames - 1)
    ...
    if 0 <= tmp < task_state.num_actions:
      action_idx = tmp
      # state.labels[state.get_image_name()] = task_state.actions[int(action_idx)]
      state.labels[state.get_image_name()] = (task_state.actions[int(action_idx)], state.gaze)
      state.save()


  # elif key == ord('r'):  # remove
  #   tmp = get_user_input('Enter the Remove Clip ID: ') - 1
  #   if 0 <= tmp < len(state.clips):
  #     state.clips.pop(tmp)

  elif key == ord('i'):  # jump to image
    tmp = get_user_input('Enter the Frame ID: ') - 1
    if 0 <= tmp < state.num_frames:
      state.image_idx = tmp
